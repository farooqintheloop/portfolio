...
const SkillOrbit = () => {
  ...
  useEffect(() => {
    ...
    const orbits = [
      { radius: 100, speed: speed * 1.3 },
      { radius: 160, speed: speed * 0.9 },
      { radius: 220, speed: speed * 0.6 }
    ];
    ...
    const animate = () => {
      ...
      skillElements.forEach((element, index) => {
        ...
        const skillsInThisOrbit = skillElements.filter(
          el => parseInt(el.dataset.orbit) === orbitIndex
        ).length;

        const indexInOrbit = skillElements.filter(
          (el, i) =>
            parseInt(el.dataset.orbit) === orbitIndex && i < index
        ).length;

        const angleOffset =
          (indexInOrbit * (2 * Math.PI)) / skillsInThisOrbit;
        ...
        const scale =
          0.8 + (0.2 * (1 + Math.sin(currentAngle)) / 2);
    ...
    <div className="flex flex-col lg:flex-row gap-12 items-center justify-between">
      ...
      <div className="w-full lg:w-1/2 space-y-8">
        ...
        <div className="bg-gray-800/40 rounded-lg p-6 backdrop-blur-sm border border-gray-700/50">
          <h3 className="text-xl text-[#00FFFF] mb-6">
            Core Languages
          </h3>

          {skills
            .filter(skill =>
              ['Java', 'JavaScript', 'Python'].includes(skill.name)
            )
            .map((skill, index) => (
        ...
        {/* Frameworks & Tools */}
        <div className="bg-gray-800/40 rounded-lg p-6 backdrop-blur-sm border border-gray-700/50">
          <h3 className="text-xl text-[#00FFFF] mb-4">
            Frameworks & Tools
          </h3>

          <div className="flex flex-wrap gap-6">
            {['Node.js', 'Express.js', 'Firebase', 'MongoDB'].map(
              (name, idx) => {
        ...
        <div className="bg-gray-800/40 rounded-lg p-6 backdrop-blur-sm border border-gray-700/50">
          ...
          <div className="flex flex-wrap gap-3">
            {['Git', 'GitHub', 'React', 'HTML5', 'CSS3'].map(
              (name, idx) => {
                const skill = skills.find(s => s.name === name);
